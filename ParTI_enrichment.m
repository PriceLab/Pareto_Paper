function [arc, arcOrig, pc, errs, pval] = ParTI(DataPoints,algNum,dim,DiscFeatName,EnMatDis,cols,ContFeatName,EnMatCont,GOcat2Genes,binSize,OutputFileName, n_arc, f_vars)
%% Inputs
...
%try
    ...
    if iscell(EnMatDis) || ~isequal(unique(EnMatDis),[0;1])
        if cols>0
            [EnMatDis, DiscFeatName] = DiscreteToBoolean(EnMatDis, DiscFeatName, cols, opf_ID);
        else
            [EnMatDis, DiscFeatName] = DiscreteToBoolean(EnMatDis, DiscFeatName, cols, opf_ID);
        end
    end
