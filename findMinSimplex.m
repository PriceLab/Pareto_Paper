function [ArchsMin,VolArchReal]=findMinSimplex(numIter,DataPCA,algNum,NArchetypes,silent, opf_ID)
...
switch algNum
    case 1 %    algNum=1 :> Sisal (default)
        if ~silent
            fprintf(opf_ID, 'Calculating archetypes positions with SISAL (Bioucas-Dias JM, 2009)\n');
        end
    ...
    case 4 %    algNum=4 :> SDVMM
        if ~silent
            fprintf(opf_ID,'Calculating archetypes positions with SDVMM (Chan T-H, Chi C-Y, Huang Y-M and Ma W-K, 2009)\n');
        end
    ...
    case 5 %    algNum=5 :> PCHA
        if ~silent
            fprintf(opf_ID,'Calculating archetypes positions with PCHA (Morup M & Hansen LK, 2011)\n');
        end
